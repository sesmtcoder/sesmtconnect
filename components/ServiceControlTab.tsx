...
import { Plus, ChevronDown, ChevronUp, FileText, Calendar, Edit2, Trash2, MapPin, Briefcase, Clock, CheckCircle2, XCircle } from 'lucide-react';
...
export default function ServiceControlTab({ processes, setProcesses, showToast }: ServiceControlTabProps) {
    ...
    const handleFileUpload = async (processId: string, event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // Em um app real, faríamos upload do arquivo para o Storage aqui
        // const fileExt = file.name.split('.').pop();
        // const filePath = `${processId}/${Math.random()}.${fileExt}`;
        // await supabase.storage.from('process_documents').upload(filePath, file);
    ...
    const handleDeleteDocument = async (processId: string, docId: string) => {
    ...
    return (
        <div className="flex h-full flex-col">
            ...
            <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h2 className="font-serif text-2xl font-bold text-slate-900">Controle de Serviços</h2>
                    <p className="text-sm text-slate-500">Acompanhamento e gestão de processos em andamento</p>
                </div>
                <button
            ...
            <div className="flex-1 overflow-auto rounded-xl border border-slate-200 bg-white shadow-sm">
                ...
                <div className="divide-y divide-slate-100">
                    {processes.length === 0 ? (
                        <div className="p-8 text-center text-slate-500">Nenhum processo cadastrado.</div>
                    ) : (
                                                    ...
                                                    <div className="p-2 flex flex-col gap-2">
                                                        {process.documents.length === 0 ? (
                                                            ...
                                                            process.documents.map(doc => (
                                                                <div key={doc.id} className="group flex items-center gap-3 rounded-lg border border-slate-100 bg-white p-3 hover:border-slate-200 transition-colors">
                                                                    <div className="flex h-8 w-8 items-center justify-center rounded bg-red-50 text-red-500">
                                                                        <FileText className="h-4 w-4" />
                                                                    </div>
                                                                    <div className="flex-1 min-w-0">
                                                                        <div className="truncate text-sm font-bold text-slate-700">{doc.name}</div>
                                                                        <div className="text-[10px] text-slate-400 mt-0.5">{doc.date} • {doc.size}</div>
                                                                    </div>
                                                                    <button
                                                                        onClick={() => handleDeleteDocument(process.id, doc.id)}
