...
const fileMap = new Map(); // lowercase -> actual
allFiles.forEach(f => {
    ...
    while ((match = importRegex.exec(content)) !== null) {
        ...
        for (const cand of candidates) {
            ...
            if (fileMap.has(lowerCand)) {
                found = true;
                actualName = fileMap.get(lowerCand).replace(/\\/g, '/');
                // Normalize both for comparison
                const normalizedCand = cand.replace(/\\/g, '/');
                if (actualName !== normalizedCand && actualName.split('.')[0] !== normalizedCand) {
        ...
        if (foundCaseMismatch) {
            ...
            console.error(`  Actual filename: @/${actualName}`);
            errors++;
        }
    }
});
